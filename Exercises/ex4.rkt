#| Exercise 4 - Currying (due Oct 20, 11:50pm)
...
(define (list-ref-keyed f x)
  (lambda (lst)
    (cond
      [(< (f x) (length lst)) (list-ref lst (f x))]
      [else (void)]))
)
...
|#
;helper function to keep subtracting n arguments one by one and call recursively
(define (curry-n f n)
  (helper f n '()))

(define (helper f n arguments)
  (if (zero? n)
        (apply f arguments)
        (lambda (h)
          (helper f (- n 1) (cons h arguments)))))
